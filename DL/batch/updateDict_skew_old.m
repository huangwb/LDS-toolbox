function newDict = updateDict_skew(data,dict,alpha,LDS_options)
...
for r=1+nAtoms/2:nAtoms
    ...
    for id_column=1:n/2
        ...
        Q_r = dict{r}.C * (kron(eye(n/2),1/sqrt(2)*[1,-1i;1,1i]));
        ...
        for id_atom=1:nAtoms
            ...
            [E_odd, dE_odd] = compute_E_skew(dict{r}.A(2*id_column-1), dict{id_atom}.A);
            [E_even, dE_even] = compute_E_skew(dict{r}.A(2*id_column), dict{id_atom}.A);
%             E_old = (1-dict{r}.A(id_column)^2)*((1-dict{id_atom}.A.^2)./(1-dict{r}.A(id_column)*dict{id_atom}.A).^2) ;
%             dE_old = 2/(1-dict{r}.A(id_column)^2)*(dict{id_atom}.A-dict{r}.A(id_column))./(1-dict{r}.A(id_column)*dict{id_atom}.A).*E;            
            
            
            tmpWC_j = W'*dict{id_atom}.C;
            tmpC_rj_odd  = Q_r(:,2*id_column-1)'*dict{id_atom}.C;
            tmpC_rj_even = Q_r(:,2*id_column)'*dict{id_atom}.C;
        ...
        for id_data=1:length(data)
            ...
            [F_1_odd, dF_1_odd] = compute_E_skew(dict{r}.A(2*id_column-1), data{id_data}.A1);
            [F_1_even, dF_1_even] = compute_E_skew(dict{r}.A(2*id_column), data{id_data}.A1);
            [F_2_odd, dF_2_odd] = compute_E_skew(dict{r}.A(2*id_column-1), data{id_data}.A2);             
            [F_2_even, dF_2_even] = compute_E_skew(dict{r}.A(2*id_column), data{id_data}.A2); 
                                       
%             F_old =  (1-dict{r}.A(id_column)^2)*((1-data{id_data}.A.^2)./(1-dict{r}.A(id_column)*data{id_data}.A).^2) ;
%             dF_old = 2/(1-dict{r}.A(id_column)^2)*(data{id_data}.A-dict{r}.A(id_column))./(1-dict{r}.A(id_column)*data{id_data}.A).*F;                   
...
%             S = S-alpha(r,id_data)*1/4*(tmpWC_i2.* (repmat(F_2_odd',[numFree+2,1]) + repmat(F_2_even',[numFree+2,1])) * tmpWC_i2');
%             dF_2 = dF_2_odd - dF_2_even;
%             dS = dS - alpha(r,id_data)*1/4*((tmpC_ri2_odd.*dF_2')*tmpC_ri2_odd' + (tmpC_ri2_even.*dF_2')*tmpC_ri2_even');            
        end
...
%         u = GCG( u0,S,20 );
        ...
        dict{r}.C(:,2*id_column-1) = dict_Cr(:,1);
        dict{r}.C(:,2*id_column) = dict_Cr(:,2);

        lambda = real(dict{r}.A(2*id_column-1)*(-1i));        
        delta = 0.001;
        a = LDS_options.SN_a;
        b = LDS_options.SN_b;
        ...
        lambda = b*(2*sigm(a*rho_new)-1);       
        
        dict{r}.A(2*id_column-1) = lambda*1i;
        dict{r}.A(2*id_column)  = -lambda*1i;
...
lambda_rk = real(theta_rk*(-1i));  % lambda_rk is real. if A_r is symmetric, dict{r}.A(k) = lambda_rk; if A_r is skew-symmetric, dict{r}.A(k) = lambda_rk*i
