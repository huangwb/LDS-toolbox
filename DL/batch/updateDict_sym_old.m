function newDict = updateDict_sym(data,dict,alpha,LDS_options)
...
for r=1:nAtoms/2
    ...
    for id_column=1:n
        ...
        numFree = m-n;
        W = [null(dict{r}.C') dict{r}.C(:,id_column)] ;
        S = zeros(numFree+1);
        dS = 0;
        ...
        for id_atom=1:nAtoms
            ...
            [E, dE] = compute_E_sym(dict{r}.A(id_column),dict{id_atom}.A);
...
%             dE_old = 2/(1-dict{r}.A(id_column)^2)*(dict{id_atom}.A-dict{r}.A(id_column))./(1-dict{r}.A(id_column)*dict{id_atom}.A).*E;            
            ...
            tmpC_rj = dict{r}.C(:,id_column)'*dict{id_atom}.C;
            S = S + tmpAlpha_rj*(tmpWC_j.*repmat(E',[numFree+1,1]))*tmpWC_j';
            dS = dS + tmpAlpha_rj*(tmpC_rj .*dE')*tmpC_rj';
        ...
        for id_data=1:length(data)
            ...
            [F_1, dF_1] = compute_E_sym(dict{r}.A(id_column), data{id_data}.A1);
            [F_2, dF_2] = compute_E_sym(dict{r}.A(id_column), data{id_data}.A2);                            
%             F1_old =  (1-dict{r}.A(id_column)^2)*((1-data{id_data}.A1.^2)./(1-dict{r}.A(id_column)*data{id_data}.A1).^2) ;
%             dF1_old = 2/(1-dict{r}.A(id_column)^2)*(data{id_data}.A1-dict{r}.A(id_column))./(1-dict{r}.A(id_column)*data{id_data}.A1).*F1_old;                   
            
            dataQ_i2 = data{id_data}.C2 * (kron(eye(n/2),1/sqrt(2)*[1 -1i;1 1i]));
            tmpWC_i1 = W'*data{id_data}.C1;
            tmpC_ri1 = dict{r}.C(:,id_column)'*data{id_data}.C1;
            tmpWC_i2 = W'*dataQ_i2;
            tmpC_ri2 = dict{r}.C(:,id_column)'* dataQ_i2; 
...
%         u = GCG( u0,S,20 );
        ...
        dict{r}.C(:,id_column) = W*u;
            
        delta = 0.001;
        a = LDS_options.SN_a;
        b = LDS_options.SN_b;
        rho_old = 1/a*(log(b+dict{r}.A(id_column))-log(b-dict{r}.A(id_column)));
        rho_new = rho_old -2*delta*dS*a*b*sigm(a*rho_old)*(1-sigm(a*rho_old));
        dict{r}.A(id_column) = b*(2*sigm(a*rho_new)-1);
        %    
    end
...
lambda_rk = theta_rk;  % lambda_rk is real. if A_r is symmetric, dict{r}.A(k) = lambda_rk; if A_r is skew-symmetric, dict{r}.A(k) = lambda_rk*i
