...
for t=1:length(DL_opts.nAtoms)
    ...
    [dict,this_costs,this_time] = learnDict_SPGD(imdb,getBlockBatch,LDS_opts,SC_opts,tDL_opts);
% uncommend the following two calls if you perform the batch method developed by
% our CVPR_2016 paper;   
%   LDS_opts.decomposition = 'SYMSKEW;
%   [dict,this_costs,this_time] = learnDict_Batch(imdb,getBlockBatch,LDS_opts,SC_opts,tDL_opts);
    ...
    for iter=1:length(trBatch)
        thisBlocks = getBlocksPerVid(imdb,trBatch(iter));
    ...
    for iter=1:length(teBatch)
        thisBlocks = getBlocksPerVid(imdb,teBatch(iter));
        thisLds = getLdsBatch(thisBlocks.data,LDS_opts);
